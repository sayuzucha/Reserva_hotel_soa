...
export class RoomsService {
  ...
  async findAll() {
    return this.roomRepo.find({ where: { isActive: 1 } });
  }

  async findOne(id: number) {
    const room = await this.roomRepo.findOne({ where: { id, isActive: 1 } });
  ...
  async blockRoom(dto: BlockRoomDto) {
    ...
    if (existing) {
      throw new ConflictException('Ya existe un bloqueo para esta reserva');
    }
  ...
  async seed() {
    ...
    if (count > 0) {
      return { message: 'Ya existe seed data' };
    }
